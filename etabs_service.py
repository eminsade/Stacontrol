...
def _get_base_bridge_url() -> str:
    """Aktif köprü URL'sini döndürür (Tünel URL'si varsa onu, yoksa yerel adresi)."""
...
def check_etabs_status() -> dict:
    ...
    try:
        data = _fetch_from_bridge("/status", timeout=2.0)
...
def get_load_combinations() -> list:
    ...
    # 1. Session State'de varsa doğrudan döndür
    ...
    # 2. Bridge'den dene
    try:
        resp = _fetch_from_bridge("/api/combinations", timeout=4.0)
...
def get_load_cases() -> list:
    ...
    try:
        resp = _fetch_from_bridge("/api/load_cases", timeout=4.0)
        if resp and resp.get("success"):
...
def get_story_names() -> list:
    ...
    try:
        resp = _fetch_from_bridge("/api/stories", timeout=4.0)
        if resp and resp.get("success"):
...
def get_column_bundle(combo: str, ts500_combo: str = "") -> dict:
    ...
    # 1. Bridge'den dene
    try:
        resp = _fetch_from_bridge("/api/column_bundle", params={"combo": combo, "ts500_combo": ts500_combo}, timeout=15.0)
        if resp and resp.get("success"):
            return {
...
def get_pier_bundle(combo: str) -> dict:
    ...
    # 1. Bridge'den dene
    try:
        resp = _fetch_from_bridge("/api/pier_bundle", params={"combo": combo}, timeout=15.0)
        if resp and resp.get("success"):
            return {
